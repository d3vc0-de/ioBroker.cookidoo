...
class CookidooAdapter extends utils.Adapter {
    ...
    async createObjects() {
        ...
        if (shopping) {
            ...
            for (const id of [
                'actions.clearShoppingList', 'actions.markAllOwned', 'actions.markAllUnowned',
                'actions.addRecipeToShopping', 'actions.removeRecipeFromShopping',
                'actions.addAdditionalItem', 'actions.removeAdditionalItem',
            ]) {
    ...
    async poll() {
        ...
        if (this.config.enableShopping !== false)          tasks.push(this.fetchShoppingList());
        if (this.config.enableCalendar !== false)          tasks.push(this.fetchCalendar());
        if (this.config.enableCustomRecipes !== false)     tasks.push(this.fetchCustomRecipes());
        if (this.config.enableManagedCollections !== false ||
            this.config.enableCustomCollections !== false) tasks.push(this.fetchCollections());
    ...
    _extractImages(assets) {
        ...
        if (!raw) return { thumbnail: null, image: null };
        return {
